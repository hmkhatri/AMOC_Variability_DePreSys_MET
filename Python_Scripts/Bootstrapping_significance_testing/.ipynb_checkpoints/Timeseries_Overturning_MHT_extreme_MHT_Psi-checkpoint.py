...
ppdir = "/gws/nopw/j04/snapdragon/hkhatri/DePreSys4/Data_Composite/Overturning_MHT/"
...
tim1 = ds_NAO['time_val'].isel(start_year=0).drop('start_year')
...
for case in case_list:
    ...
    file_count = len(glob.glob(ppdir + ensemble + "/Composite_Overturning_MHT/*" + case + "*nc")) 
    
    for k in range(0,file_count):

        d = xr.open_dataset(ppdir + ensemble + "/Composite_Overturning_MHT/Composite_" 
                            + case + "_Overturning_MHT_comp_" + str(k+1) + ".nc", chunks={'time':1})
        ds.append(d)
    ...
    for var1 in var_list:
        ...
        ds_mean[var1] = Moving_mean(ds1[var1].drop('time'), tim1, win_month)
    ...
    ds_save['latitude'] = ds['j_c'].isel(j_c=slice(0,303))
    ...
    for var1 in var_list:
        ...
        ds_save[var1 + '_standard_error'] = ds_mean[var1].std('comp') / np.sqrt(len(ds_mean['comp']))
    ...
    save_file_path = (save_path + ensemble + "Bootstrapping/" + "Bootstrap_" + case + "_Overturning_MHT_MFT_timeseries.nc")
