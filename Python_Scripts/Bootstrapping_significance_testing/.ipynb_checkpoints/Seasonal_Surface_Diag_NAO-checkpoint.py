...
for case in case_list:
    ...
    for var in var_list:
        ...
        data_var = (ds[var].isel(time=slice(4*(tim_ind-1) + 1, 4*(tim_ind-1) + 1 + 4)) * days).sum('time')
        data_var = (data_var / days.sum('time')).compute()
        
        print(var, " in progress – ", "DJFM days = ", days.sum('time').values) 
        
        # compute bootstrap confidence intervals
        bootstrap_ci = data_bootstrap(data_var, cf_lev = 0.95, num_sample = 1000)
        ...
        ds_save['standard_error'] = xr.DataArray(data = bootstrap_ci.standard_error, dims=dim_list)
        ds_save['confidence_lower'] = xr.DataArray(data = bootstrap_ci.confidence_interval[0], dims=dim_list)
        ds_save['confidence_upper'] = xr.DataArray(data = bootstrap_ci.confidence_interval[1], dims=dim_list)
