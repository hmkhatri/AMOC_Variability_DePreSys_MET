...
for year in range(year1, year2, 1):
    ...
    for r in range(1,11):
        ...
        with xr.open_mfdataset(ppdir+"Transport_sigma_1961_r" + r + "_*.nc", combine="nested", concat_dim='time',
                              chunks={'time':1, 'sigma0':1, 'sigma0_bnds':1}, parallel=True) as ds:
            ds = ds
        ...
        # regrid data
        tmp = ds.rename({'latitude_v':'lat', 'longitude_v':'lon', 'j_c':'lat','i':'lon'}).drop(['mask_North_Atl', 'dx_v'])
        ds1 = regridder(tmp)
        ...
        ds1 = ds1.astype(np.float32).persist()
