...
def processDataset(S, T, year1, year2, lead_year):
    ...
    for year in range(year1, year2):
        ...
        tmp_S = S.sel(start_year = year - 
                      lead_year).isel(time=slice(12*lead_year, 
                                                 np.minimum(12 + 12*lead_year, len(S['time']))))
        tmp_theta = T.sel(start_year = year - 
                          lead_year).isel(time=slice(12*lead_year, 
...
year1, year2 = (1979, 1985) #2017) # range over to compute average using DCPP 2016 paper
...
for var in var_list:
    ...
    for r in range(1,2):
        ...
        for year in range(year1-10, year2, 1):
            ...
            var_path = "s" + str(year) +"-r" + str(r+1) + "i1p1f2/Omon/" + "tos" + "/gn/latest/"
            d1 = xr.open_mfdataset(ppdir + var_path + "*.nc")

            var_path = "s" + str(year) +"-r" + str(r+1) + "i1p1f2/Omon/" + "sos" + "/gn/latest/" 
        ...
        for lead_year in range (0,11):
            ...
            save_file = (save_path +"Drift_" + var + "_r" + str(r+1)+ "_Lead_Year_" +
                         str(int(lead_year+1)) + ".nc")
