...
P_south = ((psl_anom * dA).sel(lat = slice(36., 40.), lon = slice(332., 340.))).sum(['lat','lon']) / dA.sum()

P_north = ((psl_anom * dA).sel(lat = slice(63., 70.), lon = slice(335., 344.))).sum(['lat','lon']) / dA.sum()
...
save_file = save_path +"NAO_SLP_Anomaly.nc"
